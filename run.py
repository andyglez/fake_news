
# coding: utf-8

# In[14]:

import pandas as pd
from tensorflow import keras
from feature_extraction.manager import flat_input, calculate_metrics, get_input_shape


# In[17]:

cols = ['ID', 'TITLE', 'TEXT', 'LABEL']
data = pd.read_csv("data/fake_or_real_news.csv", names=cols, header=0)


# In[4]:

k = len(data[cols[3]]) // 2

x = list(zip(data[cols[1]], data[cols[2]]))
y = data[cols[3]]
dim = 5


# In[6]:

x_train, y_train = flat_input(x[:k], y[:k], dim)
x_validate, y_validate = flat_input(x[k:], y[k:], dim)


# In[8]:

model = keras.Sequential()
model.add(keras.layers.Dense(units=5000, activation='relu', input_dim=get_input_shape(dim)))
model.add(keras.layers.Dense(units=1000, activation='relu'))
model.add(keras.layers.Dense(units=200, activation='relu'))
model.add(keras.layers.Dense(units=40, activation='relu'))
model.add(keras.layers.Dense(units=2, activation='softmax'))
model.compile(loss=keras.losses.categorical_crossentropy,
              optimizer=keras.optimizers.SGD(lr=0.01, momentum=0.9, nesterov=True),
              metrics=['accuracy'])


# In[9]:

model.fit(x_train, y_train, epochs=10, batch_size=32)


# In[10]:

model.evaluate(x_validate, y_validate, batch_size=32)


# In[12]:

result = model.predict(x_validate, batch_size=32).tolist()
calculate_metrics(result, y_validate.tolist())


# In[13]:

[i for i in zip(result, y_validate.tolist())]


# In[ ]:



